...
contract ConfigureMetadataContract is Script {
    ...
    function run(address metadataContract) public {
        setUp();
        SlimeShopImageLayerable imageLayerable = SlimeShopImageLayerable(
            metadataContract
        );

        vm.startBroadcast(deployer);
        imageLayerable.setAttributes(traitIds, attributes);
        imageLayerable.setBaseLayerURI(baseLayerURI);
        imageLayerable.setDefaultURI(defaultURI);
    }
}
